...
/**
 * 本次加入SecondaryShardFilterStage的目的是为了解决Secondary查询结果中过滤不属于当前shard的数据， 
 */ 

class SecondaryShardFilterStage final : public PlanStage {
public:
    SecondaryShardFilterStage(OperationContext* opCtx,
                     std::shared_ptr<ChunkManagerEx> chunkManager,
                     WorkingSet* ws,
                     PlanStage* child);
    ~SecondaryShardFilterStage();

    bool isEOF() final;
    StageState doWork(WorkingSetID* out) final;

    StageType stageType() const final {
        return STAGE_SECONDARY_SHARD_FILTER;
    }
