...
#include "mongo/db/exec/s_shard_filter.h"
...
// static
const char* SecondaryShardFilterStage::kStageType = "S_SHARDING_FILTER";

SecondaryShardFilterStage::SecondaryShardFilterStage(OperationContext* opCtx,
                                   std::shared_ptr<ChunkManagerEx> chunkManager,
                                   WorkingSet* ws,
                                   PlanStage* child)
    : PlanStage(kStageType, opCtx), _ws(ws), _manager(chunkManager) {
    _children.emplace_back(child);
}

SecondaryShardFilterStage::~SecondaryShardFilterStage() {}

bool SecondaryShardFilterStage::isEOF() {
    return child()->isEOF();
}

PlanStage::StageState SecondaryShardFilterStage::doWork(WorkingSetID* out) {
    ...
    if (PlanStage::ADVANCED == status) {
        ...
        // aborted migrations
        if (_metadata) {
            ShardKeyPattern shardKeyPattern(_manager->getShardKeyPattern());
            ...
            if (shardKey.isEmpty()) {
                ...
                warning() << "no shard key found in document " << redact(member->obj.value()) << " "
                          << "for shard key pattern " << _manager->getShardKeyPattern() << ", "
                          << "document may have been inserted manually into shard";
            }
            static BSONObj s_Empty;
            if (!_manager->findIntersectingChunk(shardKey, s_Empty)) {
                _ws->free(*out);
...
unique_ptr<PlanStageStats> SecondaryShardFilterStage::getStats() {
    _commonStats.isEOF = isEOF();
    unique_ptr<PlanStageStats> ret =
        make_unique<PlanStageStats>(_commonStats, STAGE_SECONDARY_SHARDING_FILTER);
    ret->children.emplace_back(child()->getStats());
    ret->specific = make_unique<ShardingFilterStats>(_specificStats);
    return ret;
}

const SpecificStats* SecondaryShardFilterStage::getSpecificStats() const {
